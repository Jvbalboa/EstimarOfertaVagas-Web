...
@Named 
@ViewScoped
public class CopyOfCadastroGradeController implements Serializable {
	...
	public void incluiPreRequisitos (){
		...
		int idPreprocurado = 0;

		for(PreRequisito preRequisitoQuestao :linhaSelecionada.getGradeDisciplina().getPreRequisito()){

			if (preRequisitoQuestao.getDisciplina().getCodigo().equals(disciplinaPre.getCodigo()) ){
		...
		if(disciplinaPre.getCodigo().equals(linhaSelecionada.getDisciplina().getCodigo()) ){
			FacesMessage msg = new FacesMessage("Não possível incluir como pré-requisito a própria disciplina!!");
			FacesContext.getCurrentInstance().addMessage(null, msg);
			disciplinaPre = new Disciplina();
			tipoPre = "";
			return;
		...
		linhaSelecionada.getGradeDisciplina().getPreRequisito().add(preRequisito);
		int achou = listaPreRequisitos.indexOf(preRequisito);
		if (achou < 0) listaPreRequisitos.add(preRequisito);
		disciplinaPre = new Disciplina();
		tipoPre = "";
		FacesMessage msg = new FacesMessage("Pré-Requisito incluído com sucesso!");
