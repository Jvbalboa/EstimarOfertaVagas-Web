...
import model.estrutura.DisciplinaPlanejamento;
...
public class PlanejamentoFormaturaController implements Serializable {
	...
	public boolean moverDireita(int posicao,DisciplinaPlanejamento disciplinaSelecionadaIntera ){
		...
		if (recuperarLista(posicao + 1) == null){
			FacesMessage msg = new FacesMessage("N‹o Ž poss’vel realizar esta a‹o,verifique os requisitos desta disciplina!");	
			FacesContext.getCurrentInstance().addMessage(null, msg);
			return false;
		}
		if (funcionou == false){
	...
	public void gerarExpectativa(){
		...
		if (curriculumAluno.getMandatories().keySet().size() > 12){
			FacesMessage msg = new FacesMessage("A Quantidade de Horas Per’odo deve ser maior pois o nœmero de per’odos excedeu 12 per’odos!");
			FacesContext.getCurrentInstance().addMessage(null, msg);
			return;
		}
		...
		while(horasFaltamEletivas > 0 || horasFaltamOpcionais > 0){
			...
			if (ultimoPeriodoPreenchido >= 12){
				FacesMessage msgs = new FacesMessage("A Quantidade de Horas Per’odo deve ser maior pois o nœmero de per’odos excedeu 12 per’odos!");
