...
import model.estrutura.AlunoSelecionado;
import model.estrutura.ListaPeriodoAluno;
import model.estrutura.PeriodoAluno;
import model.estrutura.TotalizadorCurso;
...
public class GraficosController implements Serializable {
	...
	private HorizontalBarChartModel initBarModel() {
		...
		while(i != (maximoEncontrado + 1)){
			...
			f.setLabel(String.valueOf(i) + "º - Período Incompleto");
			...
			//if(i != curso.gr)
			gChartModelBuilder.addRow(String.valueOf(i) + "º - Período Incompleto", contadorAluno);
			model.addSeries(f);
			i++;
		}
